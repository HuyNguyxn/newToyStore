...
public class CartFacade {
    ...
    public CartFacade(CartService cartService, ProductFacade productFacade,
    ...
    public CartResponse addItem(Integer userId, AddCartItemRequest request) {
        Product product = productFacade.getProductEntity(request.getProductId());
        ProductVariant variant = findVariant(product, request.getProductId(), request.getVariantId());

        int availableQuantity = variant.getInventory() != null
                ? variant.getInventory().getAvailableQuantity()
                : 0;
    ...
    public com.example.new_toy_store.order.application.dto.response.OrderResponse checkout(Integer userId, CheckoutCartRequest request) {
        ...
        com.example.new_toy_store.order.application.dto.response.OrderResponse createdOrder = orderFacade.createFromCart(orderRequest, cart.getId());
        return createdOrder;
    }
