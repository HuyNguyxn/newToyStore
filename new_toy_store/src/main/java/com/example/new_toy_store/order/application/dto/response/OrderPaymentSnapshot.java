...
public class OrderPaymentSnapshot {
    ...
    public OrderPaymentSnapshot(Integer orderId, Integer userId, OrderStatus status, double payableAmount, double costAmount) {
        this.orderId = orderId;
        this.userId = userId;
        this.status = status;
        this.payableAmount = Math.max(0.0, Math.round(payableAmount * 100.0) / 100.0);
        this.costAmount = Math.max(0.0, Math.round(costAmount * 100.0) / 100.0);
    }
