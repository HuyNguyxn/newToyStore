...
public class CustomerPaymentRefund extends BaseRootEntity {
    ...
    public CustomerPaymentRefund(
        ...
        if (paymentId == null) throw new InvalidCustomerCustomerPaymentDataException("paymentId", "Payment id must not be empty.");
        ...
        if (refundCode == null || refundCode.trim().isEmpty()) {
            throw new InvalidCustomerCustomerPaymentDataException("refundCode", "Refund code must not be empty.");
        }
        if (method == null) throw new InvalidCustomerCustomerPaymentDataException("refundMethod", "Refund method must not be empty.");
        if (amount <= 0) throw new InvalidCustomerCustomerPaymentDataException("amount", "Refund amount must be greater than 0.");
        if (reason == null || reason.trim().isEmpty()) {
            throw new InvalidCustomerCustomerPaymentDataException("reason", "Refund reason must not be empty.");
        }
    ...
    private void changeStatus(RefundStatus nextStatus) {
        if (!this.status.canTransitionTo(nextStatus)) {
            throw InvalidCustomerCustomerPaymentOperationException.invalidRefundTransition(this.id, this.status, nextStatus);
        }
    ...
    public boolean equals(Object o) {
        return this == o || (o instanceof CustomerPaymentRefund other && id != null && id.equals(other.id));
    }
