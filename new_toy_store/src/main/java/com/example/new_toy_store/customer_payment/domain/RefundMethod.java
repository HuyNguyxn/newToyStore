...
public enum RefundMethod {
    ...
    public static RefundMethod from(String value) {
        if (value == null || value.trim().isEmpty()) {
            throw new InvalidCustomerCustomerPaymentDataException("refundMethod", "Refund method must not be empty.");
        }
        try {
            return RefundMethod.valueOf(value.trim().toUpperCase());
        } catch (IllegalArgumentException ex) {
            throw new InvalidCustomerCustomerPaymentDataException("refundMethod", "Refund method [" + value + "] is invalid.");
        }
    }
}
