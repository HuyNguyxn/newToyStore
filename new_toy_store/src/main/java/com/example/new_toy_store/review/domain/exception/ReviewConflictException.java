...
public class ReviewConflictException extends RuntimeException {
    ...
    public static ReviewConflictException duplicateReview(Integer orderItemId) {
        return new ReviewConflictException("Bạn đã đánh giá sản phẩm này trong đơn hàng rồi. Vui lòng sử dụng tính năng chỉnh sửa.", "ACTIVE_DUPLICATE", orderItemId);
    }

    public static ReviewConflictException softDeletedConflict(Integer orderItemId) {
        return new ReviewConflictException("Đánh giá cho sản phẩm này đã từng bị xóa. Không thể tạo mới trực tiếp trùng lặp.", "SOFT_DELETED_CONFLICT", orderItemId);
    }
