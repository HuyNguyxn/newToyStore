...
function CustomerVnpayReturnPage() {
  ...
  useEffect(() => {
    ...
    handleCustomerVnpayReturn(location.search)
      .then((response) => {
        if (active) {
          setResult(response);
        }
  ...
  async function handleRetryVnpayPayment() {
    ...
    try {
      const payment = await checkoutCustomerPayment({
        orderId: orderId,
        method: 'VNPAY',
        idempotencyKey: createCustomerPaymentIdempotencyKey(orderId, 'VNPAY'),
      });
...
export default CustomerVnpayReturnPage;
