  ...
  async function selectPayment(payment) {
    ...
    try {
      const [detail, refundPage] = await Promise.all([
        ...
        getAdminPaymentRefunds(payment.id, { page: 0, size: 10, sort: 'createdAt,desc' }),
      ]);
      setSelected(detail);
        ...
        {selected && (
          <aside style={{ background: '#ffffff', borderRadius: 12, border: '1px solid #e2e8f0', padding: 24, boxShadow: '0 4px 16px rgba(0,0,0,0.02)', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gap: 8 }}>
              ...
              {canMarkSucceeded && (
                <button type="button" onClick={() => doAction(() => markPaymentSucceeded(selected.id, providerTransactionId), 'Đã xác nhận giao dịch thành công.')} style={{ width: '100%', padding: 11, background: '#16a34a', color: '#ffffff', border: 'none', borderRadius: 8, fontSize: 13.5, fontWeight: 900, cursor: 'pointer' }}>
                  Xác nhận thành công
                </button>
              )}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {canMarkFailed && <button type="button" onClick={() => doAction(() => markPaymentFailed(selected.id, reason), 'Đã cập nhật giao dịch thất bại.')} style={{ padding: 9, background: '#ffffff', color: '#dc2626', border: '1px solid #fecaca', borderRadius: 8, fontSize: 12.5, fontWeight: 800, cursor: 'pointer' }}>Thất bại</button>}
                {canCancel && <button type="button" onClick={() => doAction(() => cancelAdminPayment(selected.id, reason), 'Đã hủy giao dịch.')} style={{ padding: 9, background: '#ffffff', color: '#475569', border: '1px solid #cbd5e1', borderRadius: 8, fontSize: 12.5, fontWeight: 800, cursor: 'pointer' }}>Hủy bỏ</button>}
              </div>
              {canDeleteSelected && <button type="button" onClick={() => doAction(() => deleteAdminPayment(selected.id), 'Đã xóa giao dịch thanh toán.')} style={{ width: '100%', padding: 10, background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: 8, fontSize: 13, fontWeight: 900, cursor: 'pointer' }}>Xóa giao dịch</button>}
            </div>
